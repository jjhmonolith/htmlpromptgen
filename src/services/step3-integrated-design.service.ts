...
[HARD RULES] 🚨 Step4 검증 통과를 위한 필수 규칙
...
🧠 **교육적 효과성 필수 준수:**
- 제목 컴포넌트 필수: 시각적 계층 구조 확보 (heading 20% 이상)
- 인지 부하 관리: 핵심 개념 3-5개만 선별, 과도한 텍스트 금지
- 학습 목표 집중: 한 페이지당 핵심 메시지 1-2개로 제한
- 시각적 우선: 복잡한 설명보다 다이어그램/차트 활용
...
- "${page.topic}" 주제의 핵심 개념을 시각적으로 전달하는 구체적 방법 제시`;
  ...
  private getContentLimits(layoutMode: 'fixed' | 'scrollable', pageTopic?: string) {
    ...
    if (layoutMode === 'fixed') {
      return {
        warning: "\n🚨 **CRITICAL Fixed 모드 제약**: 1000px 높이 제한을 준수하기 위한 엄격한 콘텐츠 제한",
        textLimits: "H1(8-12자), H2(10-18자), paragraph(30-80자), card(20-50자)",
        componentLimits: `🎯 **Step4 검증 통과를 위한 필수 제한:**
- 전체 컴포넌트: 최대 5-7개 (교육적 인지 부하 관리)
- 제목(heading): 최소 1개 필수 (시각적 계층 구조)
- 제목:본문 비율 = 1:2~3 (heading 20% 이상 유지)
- paragraph: 최대 2개, 각 80자 이하
- 이미지: 최대 2개 (성능 최적화)
- card: 핵심 요점만 1개 권장
- 🧠 한 화면당 학습 목표 1-2개로 제한`,
        detailedGuide: `- H1 제목: "${topic}" 핵심 주제 (8-12자, 간결)
- H2 소제목: 핵심 포인트만 (10-18자, 최대 1개)
- paragraph: 핵심 설명만 (30-80자, 최대 2개)
- card: 요점 정리 (20-50자, 1개만)
- caption: 이미지 설명 (10-20자)
- 시각자료 우선: 복잡한 설명 → 다이어그램으로 대체`
      };
    } else {
      return {
        warning: "\n📜 **스크롤형 레이아웃**: 세로 스크롤이 가능하므로 적당한 분량의 콘텐츠를 제공합니다.",
        textLimits: "H1(10-15자), H2(15-25자), paragraph(50-120자), card(30-70자)",
        componentLimits: `📊 **교육적 최적화 가이드:**
- 전체 컴포넌트: 6-12개 (인지 부하 12개 이하)
- 제목:본문 비율 = 1:3~4 (시각적 계층 구조)
- paragraph: 3-4개까지, 각 120자 이하
- 이미지: 최대 3개 (다양한 시각 자료)
- 섹션별 독립성과 연결성 동시 고려`,
...
🚨 **CRITICAL 높이 제한 준수:**
- 전체 높이 계산: (섹션 수 × 250px) + 200px < 1000px
- 섹션을 3-4개로 엄격히 제한 (4개 = 1200px로 초과 위험)
- 각 섹션의 gapBelow를 최소화 (24-32px만 사용)
- 🎯 교육 목표: 핵심 개념 3-5개만 선별하여 구성
- 8+4 그리드 사용 시 좌우 공간 배분 최적화

🧠 **인지 부하 최적화:**
- 한 화면에 학습 목표 1-2개로 제한
- 제목(Title) : 본문(Content) = 1:2~3 비율 유지
- 복잡한 개념은 시각자료(다이어그램/차트)로 대체
- 텍스트 컴포넌트 수 최소화

**권장 섹션 구성:**
- Title 섹션 (1개): 80-120px 예상
- Content 섹션 (2-3개): 각 200-250px 예상
- 안전 여백: 200px (상단 80px + 하단 120px)
- 총 계산 예시: 120 + (3×230) + 200 = 1010px (⚠️ 위험)`;
    } else {
...
🧠 **교육적 설계 원칙:**
- 제목:본문 = 1:3~4 비율로 시각적 계층 구조 강화
- 섹션별 독립적 완결성과 연결성 동시 고려
- 점진적 정보 공개로 학습 효과 극대화
