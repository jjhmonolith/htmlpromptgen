    ...
    const getContentModeStrategy = (mode: string): string => {
      switch (mode) {
        case 'enhanced':
          return `
### 🎯 Enhanced 모드 (AI 보강)
- **HTML/CSS 시각적 요소 추가**: 차트, 그래프, 다이어그램을 HTML/CSS로 구현
- 텍스트 요약하고 HTML 기반 인포그래픽으로 보완
- 여백과 타이포그래피로 시각적 완성도 향상
- CSS 애니메이션과 인터랙션으로 학습 효과 극대화
- **이미지 사용 금지**: 실제 사진이 아닌 모든 시각화는 HTML/CSS로 처리`;
        case 'restricted':
          return `
...
- 긴 텍스트는 여러 컬럼으로 분할
- 기존 내용의 가독성 최대화
- **HTML/CSS만 사용**: 어떤 시각화도 이미지로 대체하지 않음`;
        case 'original':
          return `
### 🎯 Original 모드
- 원본 내용 최대한 보존
...
- 내용 변경 없이 레이아웃만 최적화
- **HTML/CSS 구현**: 모든 도표와 차트를 웹 기술로 구현`;
        default:
          return '';
      }
    };
...
3.  **HTML/CSS 우선**: 그래프, 차트, 다이어그램, 인포그래픽, 플로우차트, 표 등은 **모두 HTML/CSS로 구현**하도록 설계하세요.
4.  **이미지 최소화**: 이미지는 **실제 사진이나 대체불가능한 일러스트**에만 사용하세요. 데이터 시각화, 구조도, 개념도 등은 HTML/CSS로 구현 가능합니다.
5.  **이미지 사용 기준**: 다음 경우에만 이미지 사용 허용
   - 실제 인물/장소/사물 사진
   - 역사적 문서나 예술작품
   - 복잡한 일러스트레이션 (단순 도형/차트 제외)
6.  **HTML 구현 예시**:
   - 막대그래프 → div + CSS height
...
4.  **HTML/CSS 우선**: 그래프, 차트, 다이어그램, 인포그래픽, 플로우차트, 표, 타임라인 등은 **모두 HTML/CSS로 구현**하도록 설계하세요.
5.  **이미지 최소화**: 이미지는 **실제 사진이나 대체불가능한 일러스트**에만 사용하세요. 데이터 시각화나 구조도는 HTML/CSS로 더 효과적입니다.
6.  **이미지 사용 기준**: 다음 경우에만 이미지 사용 허용
   - 실제 인물/장소/사물/자연현상 사진
   - 역사적 문서나 예술작품
   - 복잡한 일러스트레이션 (단순 도형/차트 제외)
7.  **HTML 구현 예시**:
   - 막대그래프 → div + CSS height + flexbox
   - 원그래프 → CSS conic-gradient + clip-path
