...
END_S4`;
  ...
  private buildQualityGuidelines(layoutMode: 'fixed' | 'scrollable'): string {
    ...
    const modeSpecificGuidelines = {
      fixed: [
        '전체 페이지 높이가 1000px를 절대 초과하지 않도록 주의',
        '컴포넌트 수를 7개 이하로 제한하여 화면 효율성 극대화',
        '폰트 크기와 여백을 보수적으로 설정',
        '섹션 간격(margin-bottom)을 24-48px로 제한'
      ],
      scrollable: [
        '세로 스크롤을 전제로 여유 있는 간격 설정',
        '섹션 간격(margin-bottom)을 48-80px로 설정',
        '가독성을 위한 충분한 여백 확보',
        '컴포넌트별 명확한 시각적 구분'
      ]
    };
